...
function injectInlineStyles(html: string): string {
	...
	elements.forEach((element) => {
			...
			case 'table':
				element.setAttribute(
					...
					(element.getAttribute('style') || '') +
						'border: 1px solid var(--primary-contrast); border-collapse: collapse;'
				);
				break;
			case 'tr':
				element.setAttribute(
					'style',
					(element.getAttribute('style') || '') + 'border: 1px solid var(--primary-contrast);'
				);
				break;
			case 'th':
			case 'td':
				element.setAttribute(
					'style',
					(element.getAttribute('style') || '') +
						'border: 1px solid var(--primary-contrast); padding: 5px;'
				);
				break;
			case 'code':
				element.setAttribute(
					'style',
					(element.getAttribute('style') || '') +
						'font-family: monospace; border-radius: 5px; font-size: 1em;'
				);
				break;
		}
		...
		if (element.classList.contains('katex-html')) {
			element.setAttribute(
				'style',
				(element.getAttribute('style') || '') + 'display: inline-block;'
			);
		}
	});
